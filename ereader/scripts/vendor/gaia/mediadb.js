  ...
  function MediaDB(mediaType, metadataParser, options) {
    ...
    function initDeviceStorage() {
      ...
      statreq.onsuccess = function(e) {
        ...
        switch (stats.state) {
        case 'available':
            console.log("STAT READY for:" + media);
  ...
  function scan(media) {
    ...
    function ignore(file) {
      ...
      if (media.mimeTypes && media.mimeTypes.indexOf(file.type) === -1) {
          console.log('Ignoring file with type: ' + file.type)
          return true;
      }
      if (media.filePattern && file.name.match(media.filePattern) == null) {
          console.log('Ignoring file with name: ' + file.name);
          return true;
      }
    ...
    function quickScan(timestamp) {
      ...
      cursor.onsuccess = function() {
        ...
        if (file) {
          if (!ignore(file)) {
              console.log("WAKA QUICK! " + file.name);
    ...
    function fullScan() {
      ...
      cursor.onsuccess = function() {
        ...
        if (file) {
          if (!ignore(file)) {
              console.log("Waka FULL! " + file.name);
            dsfiles.push(file);
          }
          cursor.continue();
        }
